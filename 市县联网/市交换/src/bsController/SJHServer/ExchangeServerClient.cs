...
namespace ZIT.SJHServer.bsController.SJHServer
{
    class ExchangeServerClient
    {
        ...
        public ExchangeServerClient()
        {
            ExchangeMsgHandler MsgHandler = new ExchangeMsgHandler(this);
        ...
        public void SendMessage(string message)
		{
			try
			{
                ExchangeServer.server.Clients[this.intClientID].SendMessage(message);
                string CommandID = XmlUtil.XmlAnalysis("CommandID", message);
                if (CommandID != "SubCenterHandshake" && CommandID != "SubCenterBssHandShark")
                {
                    LogHelper.WriteNetMsgLog("Send message to ExchangeServer Client, Client DWBH is " + UnitCode + ":" + message);
                }
			}
			catch (Exception ex)
			{
