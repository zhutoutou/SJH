...
namespace ZIT.XJHServer.bsController.GpsServer
{
    class GServer
    {
        ...
        public void Start()
        {
            try
            {
                ...
                //ThreadPool.QueueUserWorkItem(new WaitCallback(SharkHands_Thread), SharkHandsInterval);

                //check business server connect status
                //ThreadPool.QueueUserWorkItem(new WaitCallback(CheckConnectedStatus_Thread), Controller.GetInstance().SharkHandsInterval);
        ...
        private void SharkHands_Thread(object e)
        {
            ...
            while (true)
            {
                try
                {
                    ...
                    if (udpClient != null)
                    {
                        string str = "[3000DWBH:" + Controller.GetInstance().strUnitCode + "*#DWMC:*#ZJM:*#TLX:JHL*#TH:*#ZBY:*#ZT:1*#LSH:*#ZBBC:*#]";
